    ...
    parallel $ do
        ...
        describe "right" $ do
            it "goes right one character" $
                ((^. cursor) <$> (right =<< begin =<< create 20 "Hello")) `shouldBe`
                Right (Cursor 1 0)
        ...
        describe "left" $ do
            ...
            it "doesn't go left one character" $
                ((^. cursor) <$> (left =<< begin =<< create 20 "Hello")) `shouldBe`
                Right (Cursor 0 0)
        describe "up" $ do
            it "goes up one row" $
        ...
        describe "endOfLine" $ do
            ...
            it "goes to end of line" $
                ...
                Right (Cursor 9 0)
            it "goes to end of line" $
                ((^. cursor) <$> (endOfLine =<< create 5 "Hello Fish")) `shouldBe`
                Right (Cursor 4 1)
        ...
        describe "setCursorFromRelativePosition" $ do
            ...
            describe "newlines" $ do
                ...
                it "works out position - new lines" $ do
                    ((^. cursor) <$>
                     (setCursorFromRelativePosition 9 =<< create 20 "Hello\n\n\nMum")) `shouldBe`
                        Right (Cursor 1 3)
                describe "spaces on end" $ do
                    ...
                    it "works out position - new lines" $ do
                        ((^. cursor) <$>
                         (setCursorFromRelativePosition 8 =<< create 20 "hello \n\n")) `shouldBe`
                            Right (Cursor 0 2)
                    it "works out position - new lines" $ do
                        ((^. cursor) <$>
                ...
                describe "errors" $ do
                    it "error" $ do
                        ((^. cursor) <$>
                         (setCursorFromRelativePosition (-1) =<< create 20 "hello\n\nh")) `shouldBe`
        ...
        describe "insert" $ do
            ...
            describe "cursor" $ do
                it "updates position" $
                    ((^. cursor) <$> (insert "x" =<< create 20 "Hello")) `shouldBe`
                    Right (Cursor 6 0)
        ...
        describe "backspace" $ do
            it "removes from end" $
                dump <$> (backspace =<< create 20 "Hello") `shouldBe` Right "Hell"
            ...
            describe "cursor" $ do
                it "updates position" $
                    ((^. cursor) <$> (backspace =<< create 20 "Hello")) `shouldBe`
                    Right (Cursor 4 0)
                it "updates position" $
        ...
        describe "dump" $ do
            ...
            describe "long-words" $ do
                ...
                it "should break" $
                    dump <$>
                    create 10 "today helloworldfish spoon" `shouldBe`
                    Right "today helloworldfish spoon"
