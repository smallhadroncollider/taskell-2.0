...
edWidget ed = w B.<=> c
  where
    ...
    w = B.showCursor Name (B.Location ( x, y )) . B.vBox
        $ B.txt . emptyLine <$> rowsToText rows

    c = B.padTop (B.Pad 1) (B.txt ("(" <> tshow x <> "," <> tshow y <> ")"))

draw :: E.EditorE -> [ B.Widget Name ]
draw editorE = case editorE of
...
appHandleEvent ed (B.VtyEvent (V.EvKey V.KRight [])) = B.continue
    (E.right =<< ed)
appHandleEvent ed (B.VtyEvent (V.EvKey V.KBS [])) = B.continue
    (E.backspace =<< ed)
appHandleEvent ed (B.VtyEvent (V.EvKey V.KEnter [])) = B.continue
    (E.insert "\n" =<< ed)
appHandleEvent _ (B.VtyEvent (V.EvKey (V.KChar '§') [])) = B.continue
    (E.create 50 "")
appHandleEvent ed (B.VtyEvent (V.EvKey (V.KChar '∆') [])) = B.continue
    (E.top =<< ed)
appHandleEvent ed (B.VtyEvent (V.EvKey (V.KChar '˚') [])) = B.continue
    (E.bottom =<< ed)
appHandleEvent ed (B.VtyEvent (V.EvKey (V.KChar '˙') [])) = B.continue
...
appHandleEvent ed (B.VtyEvent (V.EvKey (V.KChar '¬') [])) = B.continue
    (E.end =<< ed)
appHandleEvent ed (B.VtyEvent (V.EvKey (V.KChar c) [])) = B.continue
    (E.insert (T.singleton c) =<< ed)
appHandleEvent ed (B.VtyEvent (V.EvPaste bs)) = B.continue
    (E.insert (T.decodeUtf8With T.lenientDecode bs) =<< ed)
appHandleEvent ed _ = B.continue ed

appStart :: E.EditorE -> B.EventM Name E.EditorE
appStart state = do
    output <- V.outputIface <$> B.getVtyHandle
    when (V.supportsMode output V.BracketedPaste) . liftIO
        $ V.setMode output V.BracketedPaste True
    pure state

run :: RIO App ()
run = do
    let app = B.App { B.appDraw = draw, B.appChooseCursor = \_ c
            -> L.lastMaybe c, B.appHandleEvent = appHandleEvent, B.appStartEvent
            = appStart, B.appAttrMap = const $ B.attrMap V.defAttr [] }
